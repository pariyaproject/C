...
//此时因为定义类型转换是explicit声明,转换时需要显示转换static_cast<int>

//但是当表达式出现在下列位置时,显示的类型转换将被隐式地执行
//if,while 及 do语句的条件部分
//for语句头的条件表达式
//逻辑非运算符(!) (||) (&&)的运算对象
//条件运算符?:的条件表达式
//因此只有当用作某些表达的条件时会被隐式地转换
//c++11中对IO类型使用了显式地bool类型转换,如果cin的条件状态是good则函数返回true
//向bool转换的常用于条件部分,因此operator bool()一般定义为explicit

//避免有二义性的类型转换
//如果一个类中有一个或多个类型转换,则必须确保在类类型和目标类型之间只存在唯一一种转换方式
//通常情况下不要为类定义相同的类型转换,也不要在类中定义两个及两个以上转换源或转换目标是算术
//类型的转换
//比如B类有从A类的过来的构造函数,同时A类中有B类的转换运算,则会有二义性
//A a;B b = a;  //二义性
//b = a.operator B();  //使用这些可以明确 
//b = B(a)		//使用这些可以明确 
//注意不能使用强制类型转换,因为也可能出现二义性

//总结:除了显式地向bool类型的转换之外,尽量避免定义类型转换函数
//当调用重载函数时,如果两个(多个)用户定义的类型转换都提供了可行匹配,则认为这些类型转换一样好
//即使其中一个还调用了内置类型的转换,结果同上

//总之重载函数,运算符重载,构造函数会引发很多的二义性问题

//面向对象程序设计
//虚函数 virtual double net_price() const;
//派生类必须使用类派生列表,明确指出它是从哪个基类继承而来的
//其形式是一个冒号:后面紧跟以逗号分隔的基类列表,其中每个基类前面可以有访问说明符
//class YYY  :public XXX
//因为使用了public,所以可以完全把这个派生类对象当成XXX的对象来使用

//派生类必须在其内部对所有重新定义的虚函数进行声明,派生类可以在这样的函数之前加上virtual关键字
//但也可不加,c++11 允许派生类显式地注明它将使用哪个成员函数改写基类的虚函数
//具体措施是在函数的形参列表之后增加一个override关键字

//动态绑定
//通过动态绑定能用同一段代码分别处理基类和派生类的对象
//即实际传入的对象类型决定了将要调用的函数的版本
//因为函数的运行版本是由实参决定的,即在运行时选择函数的版本,所以动态绑定有被称为运行时绑定 
...
public:
	...
	{return n*price; } //返回销售总额
	virtual ~Quote() = default;  //对析构函数进行动态绑定
private:
	std::string bookNo;  //isbn号
protected:
	double price = .0;  //不打折的价格
};
//基类通常应该有一个虚析构函数,即使该函数不执行任何的实际操作

//成员函数与继承
//在c++中,基类必须将两种成员函数区分开来(一种是基类希望派生类进行覆盖的函数,另一种是基类希望派生类直接继承不改变的函数)
//对于前者,基类通过将其定义为虚函数,当使用指针或引用调用虚函数时,该调用将被动态绑定
//通过关键字virtual使得函数执行对动态绑定,任何构造函数之外的非静态函数都可是虚函数
//virtual只能出现在类内部的声明语句之前,而不能用于类外部的函数定义
...
//定义派生类,必须通过使用类派生列表,其中每个基类前面可以有(public,protected或private)
//其作用是控制派生类从基类继承而来的成员是否对派生类的用户可见
//派生类必须将继承而来的成员函数中需要覆盖的的那些重新声明
//因此bulik_quote类必须包含一个net_price成员
class Bulik_quote : public Quote {
public:
	Bulik_quote() = default;
...
//此外能将公有派生类型的对象绑定到基类的引用或指针上,因为使用了public,所以Bulk_quote的接口
//隐式包含isbn函数,同时在需要Quote的引用或指针的地方,可以使用Bulk_quote的对象

//派生类经常(不总是)覆盖它继承的虚函数,如果派生类没有覆盖其基类中的某个虚函数
//则该虚函数的行为类似于其他普通成员,派生类会直接继承其在基类中的版本

//派生类可以在它覆盖的函数前使用virtual关键字,但也不是必须,c++11允许派生类
//显式注明它使用某个成员函数覆盖了它继承的虚函数,具体是在形参列表后或const成员函数的const后
//或在引用成员函数的引用限定符后添加一个关键字override;

//一个派生类对象包含多个组成部分,一个含有派生类自己定义的成员的子对象
//以及一个与该派生类继承的基类对应的子对象,如果有多个,也对应有多个子对象 
//因为有对应的基类的组成部分,可以将派生类当作基类使用
//这种转换通常称为派生类到基类的类型转换(隐式)

//派生类构造函数
//对于基类的子对象只能使用基类的构造函数来初始化,不能自己直接初始化
Bulik_quote::Bulik_quote(const std::string &s, double p, std::size_t qty, double disc)
	:Quote(s,p),min_qty(qty),discount(disc)
{
	//将前两个参数交给基类的构造函数
	//除非特指出,否则派生类对象的基类部分会像数据成员一样执行默认初始化
	//首先初始化基类的部分,然后按照声明的顺序依次初始化派生类的成员
}

//派生类可以访问基类的公有成员和受保护成员
//派生类的作用域嵌在基类的作用域之内,因此,对于派生类的一个成员,使用派生类成员和使用基类成员(price)没区别

//继承与静态成员
//如果基类定义了一个静态成员,则在整个继承体系中只存在该成员的唯一定义,否认从基类中派生了多少个派生类
//对于每个静态成员来说都只存在唯一的实例

//派生类的声明与期货类差别不大,声明中包含类名但不包含它的派生列表
//(声明语句的目的只是令程序知晓某个名字的存在以及表示一个什么样的实体)派生列表以及定义有关的其他细节必须与类的主体一起出现

//如果想将某个类用作基类,则该类必须已经定义而非仅仅声明
//对于一个最终的派生类来说,它会继承其直接基类的成员,该直接基类的成员又含有其基类的成员,类推直到
//继承链的顶端

//有时不希望期货类继承它,或者不想考虑它是否适合作为一个基类,c++11提供了一种
//防止继承发生的方法,即在类名衙跟上一个关键字 final;
//class NoDerived final{...};  //不能被作为基类
//与内置指针一样,智能指针也支持派生类向基类的类型转换

//静态类型与动态类型
//当使用存在继承关系的类型时,必须将一个变量或其他表达式的静态类型与该表达式表示的对象的动态类型区分开
//表达式的静态类型在编译时总是已知的,它是变量声明时的类型或表达式生成的类型
//动态类型则是变量或表达式表示的内存中的对象的类型,动态类型直接运行才才可知
...
//编译器无法确定某个特定转换在运行时是否安全,这是因为编译器只能通过检查指针或引用
//的静态类型来推断是否合法,如果在基类中含有一个或多个虚函数,可以使用dynamic_cast请求
//一个类型转换,该转换的安全检查将在运行时执行,同样如果已知某个基类向派生类的转换是安全的
//则可以使用static_cast强制覆盖掉编译器的检查工作
//
//对象之间不存在类型转换  派生类向基类的自动类型转换只对指或引用类型有效
//当用一个派生类对象为一个基类对象初始化或赋值时,只有该派生类对象中的基类部分会被拷贝,移动或赋值
//它的派生类部分会被忽略掉

//从派生向基类的类型转换只对指针或引用类型有效
//基类向派生类不存在隐式类型转换
//派生类微基类的类型转换可能由于访问受限而变得不可行

//虚函数
//在c++中 当使用基类的引用或指针调用一个虚成员函数时会执行动态绑定,因为直到运行时才能知道
//到底调用了哪个版本的虚函数,所以所有虚函数都必须有定义,如果不使用某个函数,则无须为这个函数提供定义
//但必须为每个虚函数提供定义,而不管是否被用到,因为编译器也无法确定到底会使用哪个
//
//注意 动态绑定只有当我们通过指针或引用调用虚函数才会发生
//
//OOP的核心思想是多态性
//
//派生类的虚函数不用virtual声明也是virtual的,因此一旦声明过就已经确定
//一个派生类的函数如果覆盖了某个继承而来的虚函数,则它的形参类型必须与被它覆盖的基类函数完全一致
//同样返回类型也是,除非返回一个类自己的指针或引用

//final和override说明符
//c++11中可以使用override关键字来说明派生类中的虚函数,这么做使程序员的意图更加清晰.同时
//让编译器可以发现错误(如果没有覆盖已存在的虚函数,此时编译器将会报错)(参数或返回类型有误)
struct BBB
{
...
struct DDD:BBB
{
	...
	//void f2(int) override; //错误: 新的函数
	//void f3() override;		//错误: f3不是虚函数
	//void f4() override;	    //错误: 没有f4的函数
};
//只有虚函数才能被覆盖,所以编译器会拒绝f3

//当把某函数指定为final时.之后任何尝试覆盖此函数的操作都会引发错误
//
//虚函数和默认实参
//虚函数也能有默认的实参,如果某次函数调用使用了默认实参,则该实参值由本次调用的静态类型决定
//也就是说:如果通过基类的引用或指针调用函数,则使用基类中定义的默认实参,
//即使实际运行的是派生类中的函数版本也是如此

//回避虚函数的机制
//某些情况下,希望对虚函数的调用不要进行动态绑定,而是强迫执行虚函数的某个特定版本,使用作用域运算
//符可以实现这一目的
//double undiscounted = baseP ->Quote::net_price(42);
//通常情况下只有成员函数(友元)中的代码才需要使用作用域运算符来回避虚函数的机制

//如果一个派生类需要调用基类版本但没有使用作用域则可能会导致无限递归

//抽象基类
//纯虚函数
//和普通函数不一样,一个纯虚函数无须定义,通过在函数体的位置(在声明语句的分号之前)
//书写一个=0就可以将一个虚函数说明为纯虚函数,只能出现在类内部的虚函数声明语句处
//注意:也能为纯虚函数提供定义,不过函数体必须定义在类的外部

//含有纯虚函数的类是抽象基类
//抽象基类负责定义接口,不能直接创建一个抽象基类的对象,
//但可以定义抽象基类的派生类的对象,但这些类必须覆盖了纯虚函数(否则仍是一个抽象基类)
...
//****十分注意:继承可以看作将基类的代码直接拷贝到派生类一样,因此不会有同名的成员
//****返回的值要么在基类中定义的 要么在派生类中定义的


//公有,私有和受保护继承
//派生访问说明符对于派生类的对于派生类的成员(友元)能否访问其直接基类的成员没有直接的影响
//对基类成员的访问权限只与基类中的访问说明符[即都能访问受保护成员,不能访问private]
//派生访问说明符的目的是控制派生类用户(包括派生类的派生类)对于基类成员的访问权限
//而不是派生类内部的权限
//定义为private派生的话,基类中的public函数不能被一个派生对象调用,同理到派生的派生

//友元关系不能继承,派生类的友元也不能随意访问基类成员

//改变个别成员的可访问性
//通过使用using 声明 ,可以将该类的直接或间接基类中的任何可访问成员(例如非私有成员)标记出来
//using声明语句中的访问权限由该using声明语句之前的访问说明符来决定
//如果出现在private:只能被类和友元访问

//默认的继承保护级别
//struct(默认公有继承), class(默认私有继承)这是两者的唯一差别

//继承中的类作用域
//在编译时进行名字查找,也就是根据其静态类型进行查找
...
//名字的查找优先于类型检查
//声明在内层作用域的函数并不会重载在外层作用域的函数,如果派生类(内层)
//的成员与基类的某个成员同名,则会隐藏,即使派生类成员和基类成员的形参列表不一致
//也会被隐藏,一旦名字被找到,编译器就不再继续查找了,因此真正隐藏了基类的同名函数(可能会导致错误)
//除非显式地指定作用域
...
//覆盖重载的函数
//如果一个派生类(基类有重载函数)希望所有的重载版本对于它是可见的,要么覆盖掺的版本,或者一个也不覆盖
//有时仅需覆盖其中的部分,但如果需要基类中所有的重载函数可以对派生类可见,则必须全部覆盖
//一种好的解决方法是使用using声明语句,然后特定的在类中单独覆盖

//构造函数和拷贝控制
//和其他类一样,位于继承体系中的类也需要控制当其对象执行五系列操作时发生什么样的行为

//虚析构函数
//因为动态绑定的问题,delete可能造成不符的问题,使用虚析构函数可以解决
//如果基类的析构函数还是虚函数,则delete一个指向派生类对象的基类指针将产生未定义的行为
//一个基类总是需要一个析构函数

//虚析构函数将阻止合成移动操作
//如果一个类定义了析构函数,即使它通过=default的形式使用了合成的版本,编译器也不会为这个类合成移动操作
//合成拷贝控制与继承
//合成的成员还负责使用基类中对应的操作对一个对象的直接基类部分进行初始化,赋值或销毁的操作
//与合成的默认构造函数一样的流程

//派生类中删除的拷贝控制与基类的关系
//如果基类的这些函数被定义为删除的,则派生类中也是删除的

//移动操作和继承
//大多娄基类都会定义一个虚析构函数因此在默认情况下,基类通常不含有合成的移动操作,而且在它的派生类中
//也没有合成的移动操作
//如果确实需要移动构造函数则需要先在基类中定义,可以使用合成的版本,但必须显式定义
//一旦有了自己的移动操作,那么它必须同时显式定义拷贝操作
//而且除非基类的派生类中含有排斥移动的成员.否则派生类也会自动获得合成的移动操作

//和构造函数之类的不同,析构函数只需要析构自己派生类分配的资源即可,基类的部分会被隐式地销毁(类似析构列表)

//定义派生类的拷贝或移动构造函数

//与拷贝和移动构造函数一样，派生类的赋值运算符，也必须显式地为其基类部分赋值
//派生类的析构函数则不同,不需要显式调用基类的析构函数
//在构造函数中和析构函数中调用虚函数
//派生类对象的蕨类部分首先被创建,当执行基类的构造函数时,对象的派生类部分是
//未初始化的,类似的,销毁派生类对象的次序正好相反,因此当执行基类的析构函数时,派生类部分
//已经被销毁了
//因此当两个函数调用了某个虚函数,则应该执行与构造函数或析构函数所属类型相对应的虚函数版本

//继承的构造函数
//使用using Base::Base;  
//作用于构造函数时,using 声明语句将令编译器产生代码,对于基类的每个构造函数
//编译器都生成一个与之对应的派生类构造函数,
//也就是对于基类的每个构造函数,编译器都在派生类中生成一个形参列表完全相同的构造函数
//而且using 用于构造函数也不会改变构造函数的访问级别
//当一个基类的构造函数中含有默认的实参,这些实参不会被继承,相反,派生类将获得多个
//构造函数来弥补所有的情况
...
//template开头,后跟一个模板参数列表,这是一个逗号分隔的一个或多个模板参数的列表
//用小于号(<)和(>)包围,模板参数列表不能为空
//template和class没有区别
//<typename T,typename U> 多个模板参数应当有多个typename
//
//非类型模板参数
//一个非类型参数表示一个值而不是一个类型
//这时通过一个特定的类型名而不是关键字class或typename来指定非类型参数
//这些值必须是常量表达式
//例如
//template<unsigned N, unsigned M>
//int compare (const char (&p1)[N], const char (&p2)[M]) //使用引用防止数组退化为指针
//{return strcmp(p1,p2);}
//用于比较不同长度的两个数组 
//调用 compare("hi", "mom");

//类模板
//类似模板函数使用template<typename T> class XXX
//在类外声明一个有模板的成员函数时加上template 
//在一个模板类中返回类型是自身默认用使用ClassName 即可 不用ClassName<T>  同一个作用域中
//当一个类包含一个友元,与是否有模板无关
//为了引用模板的一个特定实例,必须首先声明自身
//template <typename> class BlobPtr; //前置声明
//template <typename X> friend class xxx; //所有xxx的实例都是友元
...
//通常情况下c++假定通过作用域运算符访问的名字不是类型,因此希望使用一个类型参数的类型成员
//就必须显示告诉编译器该名字是一个类型,通过使用关键字typename来实现
//template <typename T>
...
//默认模板实参
//template <typename T, typename F = less<T>> F表示可调用对象
//int compare(const T &v1,const T &v2, Ff = f())
//如果一个模板全部有默认,也要在使用时在后面加上<>空的

//成员模板
//成员模板不能是虚函数
//
//当在类外定义一个成员模板时,必须同时为类模板和成员模板提供模板参数列表,类模板在前
//
//控制实例化
//当模板被使用时才会进行实例化,这五特性意味着,相同的实例可能出现在多个对象文件中
//通过显式实例化来避免开销
//extern template declaration  //实例化声明
//template declaration //实例化定义
//当遇到extern模板声明时,不会在本文件中生成实例代码,承诺在程序期货位置有该实例化的一个
//非extern声明(定义),对于一个给定的实例化版本可能有多个extern声明,但必须只有一个定义
...
//通过在编译时绑定删除器,unique_ptr避免了间接调用删除嚣的运行时开销
//通过在运行时绑定删除器,shared_ptr使用户重载删除器更方便

//显式模板实参按由左至右的顺序与对应的模板参数匹配,只有尾部参数的显式模板实参可以忽略
//而且前提是它们可以从函数参数中推断出来
...
//{return *beg;}
//标准库的类型转换
//为了获得元素类型,可以使用标准库的类型转换模板定义在头文件type_traits中
//可以使用remove_reference来获得元素类型 remove_reference<int &> 返回 int
//remove_reference<decltype(*beg)>::type
//就可以在函数返回元素的拷贝(原本是返回一个左值)
//
//注意: type是一个类的成员,该类依赖于一个模板参数,因此必须在返回类型的声明中使用typename来
//告知编译器type表示一个类型
